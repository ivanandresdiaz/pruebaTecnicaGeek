...
import './GetPaymentInformation.scss';
import { useDispatch, useSelector } from 'react-redux';
import { getFullname, getPhotoURL } from '../../reducers/authReducer';
import useForm from '../../hooks/useForm';
import { addFirestoreCard } from '../../actions/productsActions';

const GetPaymentInformation = (props) => {
  const { carritoTotalPurchase } = props;
  ...
  const { number, name, date, cvc, rememberInfo } = formValues;

  const handleSubmit = (event) => {
    event.preventDefault();
    dispatch(addFirestoreCard(formValues));
    reset();
  };
  return (
    <div>
        ...
        <div id='main'>
          ...
          <div className='card'>
            ...
            <form className='form' onSubmit={handleSubmit}>
              ...
              <label htmlFor='name'>
                nombre
                <input onChange={handleInputChange} type='text' id='name' placeholder={fullname} name='name' value={name} />
              </label>
              <label htmlFor='date'>
                Valid Thru
                <input onChange={handleInputChange} type='text' id='date' placeholder='00/00' name='date' value={date} />
              </label>
              <label htmlFor='cvc'>
                cvc
              ...
              <label htmlFor='remember'>
                guardar informacion de mi tarjeta
                <input onChange={handleInputChange} type='checkbox' id='remember' name='rememberInfo' value={rememberInfo} />
              </label>
            </form>
          </div>
          <div className='price-container'>
            <strong>{carritoTotalPurchase}</strong>
